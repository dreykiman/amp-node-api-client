...
export const resolve_from_book = ords => {
  ords.forEach( ord => {
    ...
    if (added) added.resolve({event: {payload: orderbook[ord.hash]}})
    if (cancelled) cancelled.resolve({event: {payload: orderbook[ord.hash]}})
  })
}
...
export const reject_on_error = pld => {
  ...
  if (cancelled) cancelled.reject(message)
  if (added) added.reject(message)
